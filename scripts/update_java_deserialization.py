...
# -*- coding: utf-8 -*-
"""
java-deserialization 模块 index.md 自动生成脚本
优化版：使用 common.py 公共模块
多笔记汇总 + 关联 CTF/Lab/CVE
"""
...
MODULE_PATH = os.path.join(KNOWLEDGE_PATH, "05-java-security", "java-deserialization")
...
def update_module_index(dry_run: bool = False) -> bool:
    ...
    for n in notes_data:
        ...
        notes_table_lines.append(f"| [{n['title']}]({n['file']}) | {n['gadget']} | {status_emoji} |")
    ...
    practice_items = scan_practice_related(PRACTICE_PATH, tag_mapping.get('java-deserialization', []))
...
def main():
    ...
    logger.info("=" * 60)
    logger.info("java-deserialization 模块 index.md 自动生成脚本")
    logger.info("=" * 60)
