...
<span style="animation: blink 1s step-end infinite; text-shadow: 0 0 5px #ff6600, 0 0 10px #ff3300;">🎯</span> **VULN FOUND** · {finish_date}
...
def generate_index_for_note(note_dir: str, note_file: str, dry_run: bool = False) -> bool:
    ...
    info_gathering = metadata.get('info_gathering', '待补充')
    discovery = metadata.get('discovery', '待补充')
    exploit = metadata.get('exploit', '待补充')
    ...
    status_display = f"{status_emoji} **已发现**" if status_cn == '已完成' else f"{status_emoji} **挖掘中**"
    
    index_content = SRC_TEMPLATE.format(
        title=title,
        description=description,
        tags=tags,
...
def scan_and_update(dry_run: bool = False) -> None:
    """扫描并更新所有 SRC 笔记"""
    logger.info("=" * 60)
    logger.info("SRC 板块题目级 index.md 自动生成脚本")
    logger.info("=" * 60)
